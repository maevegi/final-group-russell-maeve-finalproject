...
st_crs(fire_2013)<- st_crs(new)
tm_shape(new)+tm_polygons()+
  tm_shape(fire_2013)+
  tm_dots(size="incident_acres_burned")+
  tm_layout(legend.width=0.6)

st_crs(fire_2020)<- st_crs(new)
tm_shape(new)+tm_polygons()+
  tm_shape(fire_2020)+
  tm_dots(size="incident_acres_burned")+
  tm_layout(legend.width=0.6)

st_crs(fire_2022)<- st_crs(new)
tm_shape(new)+tm_polygons()+
  tm_shape(fire_2022)+
  tm_dots(size="incident_acres_burned")+
  tm_layout(legend.width=0.6)
...
We want to also compare the trends of these different variables across the past decade by running the data points from these variables on the same graph. In this graph we are comparing the trends of total acres burned with average annual temperatures. 
```{r}
...
This graph highlights total acres burned and annual precipitation values. In general, the higher the precipitation, the wetter the land, and therefore the lesser the chance of fires occurring. This correlation is clear especially between 2019 and 2020, whereby 2019 had a way higher average precipitation rate compared to 2020. In turn, there’s a sharp increase in total acres burned. However, other years have rather ambiguous relationships, including but not limited to the years 2015 to 2017, whereby the total acres burned increased despite the increasing average precipitation.
```{r}
...
Discussion: Our results showcase how the fluctuations in climate data have potential correlations with the frequency and severity of forest fires. 

#notes:
-adding longitude and latitude points to rasters, creating raster for each year and using predictor equations 
-describe transformation applied to the data to generate secondary y-axis
-update README file
-clean up formatting and knit to pdf 
