...
NOTE: We experienced difficulty getting our tmap objects to consistently print the tm_dots onto the California sf object. Depending on how many times and the order in which our code ran, the outcome was different. We did our best to resolve the issue (see commit history) but it is likely that the tmap objects will not print correctly in the final version. Rather than deleting this section, we wanted to keep our work in the event that we can resolve the issue later, and because we spent a lot of time and effort trying to create these maps. 
